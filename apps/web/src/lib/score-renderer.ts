...
export class ScoreRenderer {
	...
	private ensureWorker(): Worker {
		if (!this.worker) {
			...
			this.worker.onmessage = (
				...
				if (error !== undefined) {
					if (pending.sentKey !== undefined)
						this.sentPieceIds.delete(pending.sentKey);
	...
	async load(
		...
		if (needsBytes && bytes === undefined) {
			throw new Error(
				`Score bytes missing after fetch for pieceId: ${pieceId}`,
			);
		}
		...
		try {
			const payload = await this.sendRequest<{
				ir: ScoreIR;
				pageSvgs: string[];
			}>(pieceId, { type: "load", transpose }, bytes, key);
	...
	async getClipPlayback(
		...
		svg: string;
		ir: ScoreIR;
		notes: import("./score-worker").ClipNote[];
	}> {
		const worker = this.ensureWorker();
		return new Promise((resolve, reject) => {
			const requestId = `req-${++this.requestCounter}`;
