...
describe("LoopPlayer", () => {
	...
	it("transitions to counting-in then playing after play()", async () => {
		...
		await player.play();
		expect(player.state === "counting-in" || player.state === "playing").toBe(
			true,
		);
		player.destroy();
	});
	...
	it("first note of pass N+1 is scheduled in the wrap-boundary tick (no dropout, no doubling)", async () => {
		...
		const SINGLE_NOTE: ClipNote[] = [{ midi: 60, startQ: 16, endQ: 18 }];
		const player = new LoopPlayer({
			ctx,
			instrumentUrl: "/soundfonts/acoustic_grand_piano-mp3.js",
			clipIR: STUB_IR,
		...
		const sfInstance = (Soundfont as ReturnType<typeof vi.fn>).mock.results[0]
			?.value;
		expect(sfInstance).toBeDefined();
		const startCalls = (sfInstance.start as ReturnType<typeof vi.fn>).mock
			.calls as Array<[{ note: number }]>;
