...
export class LoopClock {
	...
	constructor(opts: LoopClockOptions) {
		...
		if (opts.clipEndQ <= opts.clipStartQ) {
			throw new Error(
				`LoopClock: clipEndQ (${opts.clipEndQ}) must be greater than clipStartQ (${opts.clipStartQ})`,
			);
		}
	...
	private rawQAtMs(nowMs: number): number {
		...
		const elapsedSec =
			(nowMs - this.startMs!) / 1000 - this.calibrationMs / 1000;
		...
		const playbackSec =
			this.calibrationMs === 0
				...
				: elapsedSec;
		return (
			this.clipStartQ + this.phaseOriginQ + playbackSec * this.qPerSecond()
		);
	}
}
