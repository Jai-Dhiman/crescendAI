...
function extractMeasureNoteMap(
	svgText: string,
): Array<{ measureId: string; noteIds: string[] }> {
...
export function parseScoreIR(
	...
	for (let i = 0; i < measures.length; i++) {
		measureByMeasureOn.set(measures[i].measureOn, {
			qstamp: measures[i].qstamp,
			idx: i,
		});
	}
	...
	for (let pageIdx = 0; pageIdx < pageSvgs.length; pageIdx++) {
		...
		for (const [id, { x, y }] of notePositions) {
			...
			if (qstamp === undefined) {
				console.error(
					`[score-ir] note id "${id}" absent from noteQstampMap — recording NaN qstamp`,
				);
			}
			notes[id] = {
				id,
