...
pub fn ExpandableCitation(
    ...
    view! {
        <span class="inline">
            <button
                type="button"
                class="citation-marker inline-flex items-center justify-center w-5 h-5 text-xs font-medium bg-sepia-100 text-sepia-700 rounded cursor-pointer hover:bg-sepia-200 hover:scale-110 transition-all duration-150 align-baseline mx-0.5"
...
fn CitationExpansion(
    ...
    let icon = match citation.source_type {
        SourceType::Book => view! {
            <svg class="w-4 h-4 text-sepia-500 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
    ...
    view! {
        <div class="citation-expansion block mt-2 mb-3 p-3 bg-paper-50 border border-sepia-200 rounded-lg shadow-sm animate-in slide-in-from-top-2 duration-200">
            // Header with source info and close button
            <div class="flex items-start justify-between gap-2 mb-2">
                <div class="flex items-start gap-2 min-w-0">
                    {icon}
                    <div class="min-w-0">
                        <p class="text-label-sm text-sepia-600 font-medium truncate">
                            {footnote}
                        </p>
                    </div>
                </div>
                <button
                    type="button"
                    class="text-sepia-400 hover:text-sepia-600 transition-colors flex-shrink-0 p-0.5"
            ...
            {citation.quote.map(|quote| {
                view! {
                    <blockquote class="text-body-sm text-ink-500 italic border-l-2 border-sepia-200 pl-3 my-2">
                        {format!("\"{}\"", quote)}
                    </blockquote>
                }
            })}
            ...
            {url.map(|link| {
                view! {
                    <a
                        ...
                        class="inline-flex items-center gap-1 text-label-sm text-sepia-600 hover:text-sepia-800 hover:underline transition-colors"
                    >
                        "View source"
