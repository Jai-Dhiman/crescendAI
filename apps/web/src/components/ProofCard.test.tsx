		...
		exercises: [
			{
				...
				instruction: "Hold forte through beat 2",
	...
	globalThis.fetch = vi.fn().mockImplementation((url: string) => {
		...
		if (String(url).includes("score.svg")) {
			return Promise.resolve({
				...
				text: () =>
					Promise.resolve(
						'<svg xmlns="http://www.w3.org/2000/svg"><g class="measure" data-bar="4"></g></svg>',
					),
			});
		}
...
describe("ProofCard render contract", () => {
	...
	it("renders score container, audio scrubber, diagnosis text, and exercise after loading", async () => {
		...
		expect(
			document.querySelector('[data-testid="proof-card-score"]'),
		).not.toBeNull();

		// Audio scrubber
		expect(
			document.querySelector('[data-testid="proof-card-scrubber"]'),
		).not.toBeNull();

		// Diagnosis text
		await waitFor(() => {
			expect(
				screen.getByText(/The diminuendo in bar 4 arrives too early/),
			).toBeTruthy();
		});
		...
		await waitFor(() => {
			expect(
				document.querySelector('[data-testid="proof-card-exercise"]'),
			).not.toBeNull();
		});
	});
});
