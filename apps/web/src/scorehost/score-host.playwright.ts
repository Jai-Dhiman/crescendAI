...
test("ScoreHost.load + showArtifact(score_highlight) engraves note glyphs", async ({
	page,
}) => {
	await page.goto("/");
	await page.waitForFunction(
	...
	const artifactJson = JSON.stringify({
		...
		config: {
			...
			highlights: [
				{ bars: [1, 4], dimension: "dynamics", annotation: "forte" },
			],
		},
	});
	...
	const svgCount = await page.evaluate(
		() => document.querySelectorAll("#scorehost-container > div > svg").length,
	);
	expect(svgCount).toBe(1);
});

test("ScoreHost play_passage renders clip and fires playback event", async ({
	page,
}) => {
	await page.goto("/");
	await page.waitForFunction(
	...
	await page.evaluate(() => {
		...
		const origPostMessage =
			window.webkit?.messageHandlers?.scoreHostEvents?.postMessage?.bind(
		...
		(window as any).webkit.messageHandlers.scoreHostEvents = {
			postMessage: (msg: any) => {
				if (msg.type === "playback") {
					((window as any).__playbackEvents as string[]).push(
						msg.payload.state,
					);
				}
	...
	await expect(
		page.evaluate(() => (window as any).ScoreHost.play()),
	...
	const useCountAfterPlay = await page.evaluate(
		() => document.querySelectorAll("svg use").length,
	);
	expect(useCountAfterPlay).toBeGreaterThan(0);

	const transportPresent = await page.evaluate(
		() => !!document.getElementById("loop-transport"),
	);
...
test("ScoreHost.load fetches non-bundled piece via __SCOREHOST_API_BASE (/api/scores/:id/data)", async ({
	page,
}) => {
	...
	await page.route(
		"**/api/scores/chopin-nocturne-op9-no2/data",
		async (route) => {
			apiPathHit = true;
			// Fetch the czerny fixture from the running vite preview as a valid .mxl stand-in.
			const bytes = await fetch(
				"http://localhost:5173/scores/czerny-op299-no1.mxl",
			)
	...
	const artifactJson = JSON.stringify({
		...
		config: {
			pieceId: "chopin-nocturne-op9-no2",
			highlights: [
				{ bars: [1, 4], dimension: "phrasing", annotation: "sing here" },
			],
		},
	});
	await page.evaluate(async (json: string) => {
