...
const haveData =
	SRC_MEI !== "" &&
	SAMPLE !== "" &&
	fs.existsSync(SAMPLE) &&
	fs.existsSync(SRC_MEI);
...
const sample: SampleRow[] = haveData
	? JSON.parse(fs.readFileSync(SAMPLE, "utf8"))
	: [];

test("DISPLAY: stratified .mei sample engraves in real scorehost Verovio worker", async ({
	page,
}) => {
	test.skip(
		!haveData,
		"set MEI_DIR + MEI_SAMPLE (+ SHOTS_DIR) to run the .mei display sweep",
	);
	if (SHOTS) fs.mkdirSync(SHOTS, { recursive: true });
	const out = fs.createWriteStream(
		path.join(SHOTS || SRC_MEI, "mei_display_results.jsonl"),
		{ flags: "w" },
	);
	...
	await page.route("**/api/scores/*/data", async (route) => {
		...
		if (!fs.existsSync(file)) {
			await route.fulfill({ status: 404, body: "no mei" });
			return;
		}
		await route.fulfill({
			status: 200,
	...
	for (const row of sample) {
		...
		console.log(
			`${rec.ok ? "OK " : "FAIL"} ${pid}  bars=${row.total_bars} use=${rec.useCount ?? "-"} m1=${rec.page1Measures ?? "-"} ${rec.error ?? ""}`,
		);
	}
	out.end();
	...
	if (failed.length)
		console.log("FAILURES:", failed.map((r) => r.pid).join(", "));
	expect(
		failed,
		`pieces that failed to engrave: ${failed.map((r) => r.pid).join(", ")}`,
	).toEqual([]);
});
