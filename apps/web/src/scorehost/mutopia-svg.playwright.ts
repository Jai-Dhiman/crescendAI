...
test("Mutopia LilyPond .svg displays via scorehost static tier", async ({
	page,
}) => {
	...
	console.log(
		`Mutopia SVG display: svg=${stats.svgCount} paths=${stats.pathCount}`,
	);
	...
	if (SHOTS) {
		...
		await page
			...
			.screenshot({ path: path.join(SHOTS, "mutopia_svg_display.png") });
	}
});
