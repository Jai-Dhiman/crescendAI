...
const sample: Row[] = haveData
	? JSON.parse(fs.readFileSync(SAMPLE, "utf8"))
	: [];
const byPid = new Map(sample.map((r) => [r.pid, r]));

test("BAKEOFF: candidate engravings render in real scorehost Verovio worker", async ({
	page,
}) => {
	test.skip(
		!haveData,
		"set BAKEOFF_SAMPLE (+ SHOTS_DIR) to run the engine bake-off sweep",
	);
	if (SHOTS) fs.mkdirSync(SHOTS, { recursive: true });
	const out = fs.createWriteStream(
		path.join(SHOTS || ".", "bakeoff_display_results.jsonl"),
		{ flags: "w" },
	);
	...
	await page.route("**/api/scores/*/data", async (route) => {
		...
		if (!row || !fs.existsSync(row.file)) {
			await route.fulfill({ status: 404, body: "no asset" });
			return;
		}
		await route.fulfill({
			status: 200,
			contentType: "application/octet-stream",
			body: fs.readFileSync(row.file),
		});
	});

	await page.goto("/");
	await page.waitForFunction(
		() => typeof (window as any).ScoreHost !== "undefined",
		{ timeout: 10000 },
	);
	await page.evaluate(() => {
	...
	for (const row of sample) {
		...
		try {
			const loaded = await page.evaluate(
				async (id) => await (window as any).ScoreHost.load(id),
				row.pid,
			);
			...
			const artifact = JSON.stringify({
				...
				config: { pieceId: row.pid, highlights },
			});
			await page.evaluate(async (j) => {
				await (window as any).ScoreHost.showArtifact(j);
			}, artifact);
			await page.waitForSelector("#scorehost-container svg", {
				timeout: 25000,
			});
			...
			rec.ok =
				stats.svgCount >= 1 && (stats.useCount > 0 || stats.pathCount > 50);
			if (SHOTS) {
				const safe = row.label.replace(/[^a-z0-9_.-]/gi, "_");
				await page
					.locator("#scorehost-container")
					.screenshot({ path: path.join(SHOTS, `${safe}.png`) });
			}
		} catch (e: any) {
			rec.ok = false;
			...
			await page
				.evaluate(() => {
					document.getElementById("scorehost-container")!.innerHTML = "";
				})
				.catch(() => {});
		}
		...
		console.log(
			`${rec.ok ? "OK " : "FAIL"} ${row.label}  use=${rec.useCount ?? "-"} path=${rec.pathCount ?? "-"} note=${rec.noteCount ?? "-"} meas=${rec.measureCount ?? "-"} ${rec.error ?? ""}`,
		);
	}
	out.end();
});
