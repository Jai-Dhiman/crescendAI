		...
		const url =
			typeof input === "string"
				...
				: input instanceof URL
					? input.toString()
					: input.url;
		const apiBase = (window as unknown as { __SCOREHOST_API_BASE?: string })
			.__SCOREHOST_API_BASE;
...
async function fetchScoreBytes(pieceId: string): Promise<ArrayBuffer> {
	...
	if (apiBase) {
		...
		if (!res.ok) {
			throw new Error(
				`Failed to fetch score for ${pieceId}: HTTP ${res.status}`,
			);
		}
	...
	throw new Error(
		`Failed to fetch score for ${pieceId}: no .mei or .mxl asset`,
	);
}
...
async function ensureLoaded(pieceId: string): Promise<void> {
	...
	const promise = (async () => {
		...
		const payload = await sendRequest<LoadPayload>(
			{ type: "load", pieceId },
			bytes,
		);
...
async function renderPlayPassage(config: PlayPassageConfig): Promise<void> {
	...
	if (clipResult === "failed") {
		throw new Error(
			`renderPlayPassage: get_clip_playback failed for ${pieceId} bars ${startBar}-${endBar}`,
		);
	}
	...
	playBtn.style.cssText =
		"padding:6px 14px;border-radius:6px;border:1px solid #ccc;cursor:pointer;";
...
async function renderScoreHighlight(
	...
	if (config.highlights.length > 0) {
		...
		svgHtml = await sendRequest<string>({
			type: "get_clip",
			pieceId,
			startBar,
			endBar,
		...
		if (!cached) {
			throw new Error(
				`renderScoreHighlight: no cached load for pieceId: ${pieceId}`,
			);
		}
...
const ScoreHostImpl = {
	...
	async play(): Promise<void> {
		...
		if (!playBtn) {
			throw new Error(
				"play: no active loop transport — call showArtifact(play_passage) first",
			);
		}
		playBtn.click();
	},
...
(window as unknown as { ScoreHost: typeof ScoreHostImpl }).ScoreHost =
	ScoreHostImpl;
