...
use crate::types::{BarMap, FollowerState, NoteAlignment, PerfNote, ScoreBar};
...
/// Run DTW over a score window and return (path, cost).
...
///
/// Returns (Option<BarMap>, updated FollowerState).
pub fn align_chunk(
    chunk_index: usize,
    perf_notes: &[PerfNote],
    score_bars: &[ScoreBar],
    ...
    let score_notes = flatten_score_notes(score_bars);
    ...
    let max_bar = score_bars.iter().map(|b| b.bar_number).max().unwrap_or(1);
...
mod tests {
    use super::*;
    use crate::types::{ScoreBar, ScoreNote};
    ...
    fn aligns_simple_ascending_scale() {
        ...
        let result = align_chunk(0, &perf_notes, &bars, &mut state);
    ...
    fn continuity_across_chunks() {
        ...
        let mut state = FollowerState::default();
        let result1 = align_chunk(0, &chunk1_notes, &bars, &mut state);
        ...
        let result2 = align_chunk(1, &chunk2_notes, &bars, &mut state);
