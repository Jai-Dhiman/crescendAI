...
fn analyze_dynamics_tier1(
    ...
    // Score marking from tempo/dynamic markings if available
    let score_marking = score_ctx
        .score
        .tempo_markings
        .first()
        .and_then(|v| v.as_str())
...
fn analyze_pedaling_tier1(
    ...
    // Count perf pedal on/off pairs and compute average duration
...
fn analyze_articulation_tier1(
    ...
    // Build a lookup from onset -> offset for perf notes
    ...
    // Score note durations in bar range
...
mod tests {
    ...
    fn timing_detects_rushing() {
        // Negative onset deviations: performance is ahead of score = rushing
        let alignments = vec![
        ...
        let bar_map = make_bar_map(alignments);
        let notes = vec![
            make_perf_note(60, 0.0, 0.4, 80),
            make_perf_note(62, 0.5, 0.9, 80),
            make_perf_note(64, 1.0, 1.4, 80),
            make_perf_note(65, 1.5, 1.9, 80),
        ];
        let pedal = vec![];
        let scores = default_scores();
        ...
        // Confirm the timing analysis string from tier1 logic path mentions rushing
