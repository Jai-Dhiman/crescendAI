...
function makeDigest(
	sessionMeansCounts: Record<string, number>,
): GroundedDigest {
	const dims = [
		"dynamics",
		"timing",
	...
	return {
		...
		compact_signal_summary:
			"chunk:0 bars 1-8 muq=[0.55,0.48,0.46,0.54,0.52,0.51]",
		piece_id: "test-piece",
	};
}
...
test("resolveMoleculeContext: bar_range=[2,3] filters midi_notes to bars 2-3 only", async () => {
	...
	expect(
		ctx.bundle.midi_notes.every(
			(n) => n.bar !== undefined && n.bar >= 2 && n.bar <= 3,
		),
	).toBe(true);
});
...
test("resolveMoleculeContext: session_means length>=3 produces multi-session baseline", async () => {
	const ctx = await resolveMoleculeContext(
		makeDigest({ dynamics: 3, timing: 3 }),
		[2, 3],
	);
...
test("resolveMoleculeContext: null-origin [0,0] chunk + bar_range=null does not throw", async () => {
	...
	digest.compact_signal_summary =
		"chunk:0 bars 0-0 muq=[0.55,0.48,0.46,0.54,0.52,0.51]";
