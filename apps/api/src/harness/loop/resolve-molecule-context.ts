...
import type {
	CohortStats,
	GroundedDigest,
	GroundedPastDiagnosis,
} from "./grounded-digest";
	...
	const baselineEntries = await Promise.all(
		DIMENSIONS_6.map(async (dim) => {
			...
			if (session_means.length >= 3) {
				const result = (await fetchStudentBaseline.invoke({
					dimension: dim,
					session_means,
				})) as Baseline | null;
				if (result === null) {
					throw new Error(
						`resolveMoleculeContext: fetchStudentBaseline returned null despite n=${session_means.length} sessions for dim=${dim}`,
					);
				}
				...
				return [
					dim,
					{
						dimension: dim,
						mean,
						stddev: Math.max(0.1, digest.cohort[dim].stddev),
						n_sessions: session_means.length,
					},
				] as const;
			}
		}),
