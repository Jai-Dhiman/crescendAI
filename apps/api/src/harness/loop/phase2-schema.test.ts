	...
	topMoments: [
		{
			...
			deviation: -1.31,
			is_positive: false,
			reasoning: "Flatter than session mean.",
		},
	],
	drillingRecords: [],
	pieceMetadata: null,
...
const VALID_ARTIFACT: SynthesisArtifact = {
	...
	focus_areas: [
		{
			dimension: "dynamics",
			one_liner: "Dynamics flattened in the middle section.",
			severity: "minor",
		},
	],
...
function findIllegalConstructs(node: unknown, path = "$"): string[] {
	...
	if (Array.isArray(node)) {
		node.forEach((v, i) => {
			issues.push(...findIllegalConstructs(v, `${path}[${i}]`));
		});
		return issues;
	}
	if (node && typeof node === "object") {
		for (const [k, v] of Object.entries(node as Record<string, unknown>)) {
			if (k === "$ref")
				issues.push(
					`${path}.$ref (internal ref not accepted as tool input_schema)`,
				);
			if (k === "nullable")
				issues.push(
					`${path}.nullable (OpenAPI 3.0 keyword; 2020-12 wants type: [..,"null"])`,
				);
			if (k === "exclusiveMinimum" && typeof v === "boolean")
				issues.push(
					`${path}.exclusiveMinimum is boolean (2020-12 requires a number)`,
				);
			if (k === "exclusiveMaximum" && typeof v === "boolean")
				issues.push(
					`${path}.exclusiveMaximum is boolean (2020-12 requires a number)`,
				);
	...
	try {
		...
		fetchSpy.mockResolvedValueOnce(
			new Response(
				JSON.stringify({
					content: [
						{ type: "text", text: "No AMT signals; nothing to diagnose." },
					],
		...
		const body = JSON.parse(
			(phase2Call?.[1] as RequestInit).body as string,
		) as {
