...
export async function buildGroundedDigest(
	...
	const barNoteMap = new Map<
		number,
		{ onset_ms: number; duration_ms: number }[]
	>();
	...
	for (let dimIdx = 0; dimIdx < DIMENSIONS_6.length; dimIdx++) {
		...
		const vals = input.enrichedChunks
			.map((c) => c.muq_scores[dimIdx])
			.filter((v) => typeof v === "number");
		within_session_means[dim] =
			vals.length > 0 ? vals.reduce((s, v) => s + v, 0) / vals.length : 0.5;
	}
	...
	for (const row of rows) {
		...
		if (!sessionDimMap.has(row.sessionId))
			sessionDimMap.set(row.sessionId, new Map());
	...
	const past_diagnoses_grounded: GroundedPastDiagnosis[] =
		input.pastDiagnoses.map((r) => ({
			artifact_id: r.id,
			session_id: r.sessionId,
			created_at: new Date(r.createdAt).getTime(),
			primary_dimension: r.primaryDimension,
			bar_range:
				r.barRangeStart !== null && r.barRangeEnd !== null
