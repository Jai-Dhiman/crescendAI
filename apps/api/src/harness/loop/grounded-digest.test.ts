...
test("buildGroundedDigest: bar assigned from chunk bar_coverage[0] (chunk-level approximation)", async () => {
	...
	const digest = await buildGroundedDigest(
		makeInput(),
		{ db: mockDb, studentId: "stu-1" },
		COHORT_TABLES,
	);
...
test("buildGroundedDigest: carries reference_mode from input (cold-start guardrail gate)", async () => {
	...
	const within = await buildGroundedDigest(
		{ ...makeInput(), referenceMode: "within_session" },
		{ db: mockDb, studentId: "stu-1" },
		COHORT_TABLES,
	);
	expect(within.reference_mode).toBe("within_session");
	const returning = await buildGroundedDigest(
		{ ...makeInput(), referenceMode: null },
		{ db: mockDb, studentId: "stu-1" },
		COHORT_TABLES,
	);
