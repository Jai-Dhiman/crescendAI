...
export function buildCompact10ChunkDigest(): GroundedDigest {
	...
	const chunks = Array.from({ length: 10 }, (_, i) => ({
		...
		midi_notes: Array.from({ length: 200 }, (_, j) => ({
			pitch: 60,
			onset_ms: j * 100,
			duration_ms: 400,
			velocity: 70,
	...
	const lines = chunks.map(
		(c) =>
			`${c.chunk_id} bars ${c.bar_coverage[0]}-${c.bar_coverage[1]} muq=[${c.muq_scores.map((v) => v.toFixed(2)).join(",")}]`,
	);
