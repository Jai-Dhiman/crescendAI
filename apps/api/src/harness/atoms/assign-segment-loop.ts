...
export async function assignSegmentLoopAtom(
	...
	if (!ctx.trigger) {
		throw new ToolPreconditionError(
			"assign_segment_loop requires trigger on PhaseContext",
		);
	}
...
export const ASSIGN_SEGMENT_LOOP_TOOL: ToolDefinition = {
	...
	input_schema: {
		...
		properties: {
			...
			bars_start: {
				type: "integer",
				description:
					"First bar of the practice passage (inclusive, 1-indexed).",
				minimum: 1,
			},
			bars_end: {
				type: "integer",
				description:
					"Last bar of the practice passage (inclusive). Must be >= bars_start.",
				minimum: 1,
			},
			required_correct: {
				type: "integer",
	...
	invoke: async (input: unknown, ctx?: PhaseContext): Promise<unknown> => {
		if (!ctx)
			throw new ToolPreconditionError(
				"assign_segment_loop requires PhaseContext",
			);
