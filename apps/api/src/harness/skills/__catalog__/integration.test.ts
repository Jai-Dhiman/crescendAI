...
const FIXTURE_CHUNK = {
	chunkIndex: 0,
	muq_scores: [0.55, 0.48, 0.62, 0.51, 0.58, 0.6],
	midi_notes: [
...
const VALID_SYNTHESIS_ARTIFACT: SynthesisArtifact = {
	...
	focus_areas: [
		{
			dimension: "pedaling",
			one_liner: "Pedal releases were slightly late at bars 3-4.",
			severity: "minor",
		},
	],
...
test("E2E: extract-bar-range-signals atom invoked via real runHook pipeline", async () => {
	...
	try {
		...
		expect(toolResultEv).toMatchObject({
			type: "phase1_tool_result",
			ok: true,
			tool: "extract-bar-range-signals",
		});

		// SignalBundle output has muq_scores: number[][] — one 6-vector per chunk
		if (
			toolResultEv &&
			toolResultEv.type === "phase1_tool_result" &&
			toolResultEv.ok
		) {
