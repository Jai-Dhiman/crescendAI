...
export const dynamicRangeAudit: ToolDefinition = {
	...
	description:
		"Compares velocity range used in performance against cohort/baseline dynamics. score_marking_type is not available and is not used.",
	input_schema: {
		type: "object",
		properties: {
			bar_range: {
	...
	invoke: async (
		...
		if (!ctx)
			throw new Error(
				"dynamic-range-audit: ctx (PhaseContext with digest) is required",
			);
