...
function projectVoices(
	notes: { pitch: number; velocity: number; bar: number }[],
	bar: number,
) {
...
export const voicingDiagnosis: ToolDefinition = {
	...
	description:
		'Diagnoses melody/accompaniment voicing imbalance in homophonic textures. Returns DiagnosisArtifact with primary_dimension "dynamics".',
	input_schema: {
		type: "object",
		properties: {
			bar_range: {
	...
	invoke: async (
		...
		if (!ctx)
			throw new Error(
				"voicing-diagnosis: ctx (PhaseContext with digest) is required",
			);
		...
		const muq_dynamics =
			ctx_r.bundle.muq_scores.length > 0
				? ctx_r.bundle.muq_scores.reduce((s, row) => s + row[0], 0) /
		...
		const bars = [...new Set(ctx_r.bundle.midi_notes.map((n) => n.bar))].sort(
			(a, b) => (a ?? 0) - (b ?? 0),
		);
		let flatCount = 0;
		for (const bar of bars) {
			const p = projectVoices(
				ctx_r.bundle.midi_notes as {
					pitch: number;
					velocity: number;
					bar: number;
				}[],
		...
		return DiagnosisArtifactSchema.parse({
			...
			one_sentence_finding:
				"Melody and accompaniment are voiced almost equally; the top line is not coming through.",
