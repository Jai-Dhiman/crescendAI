...
export const phrasingArcAnalysis: ToolDefinition = {
	...
	description:
		"Assesses dynamic and timing arc shape across a complete phrase by detecting peak position and drift convergence.",
	input_schema: {
		type: "object",
		properties: {
			bar_range: {
	...
	invoke: async (
		...
		if (!ctx)
			throw new Error(
				"phrasing-arc-analysis: ctx (PhaseContext with digest) is required",
			);
		...
		const muq_phrasing =
			ctx_r.bundle.muq_scores.length > 0
				? ctx_r.bundle.muq_scores.reduce((s, row) => s + row[4], 0) /
		...
		const flatOrMulti =
			peakIndex === 0 || peakIndex === curve.length - 1 || nearPeak.length > 1;
		...
		if (ctx_r.bundle.alignment.length > 0) {
			...
			lastDrift =
				drift.length > 0 ? Math.abs(drift[drift.length - 1].signed) : 0;
		}
		...
		if (!flatOrMulti && lastDrift <= 50) {
			return DiagnosisArtifactSchema.parse({
				...
				one_sentence_finding:
					"The phrase has a clear arc with a well-placed peak.",
				confidence: "medium",
