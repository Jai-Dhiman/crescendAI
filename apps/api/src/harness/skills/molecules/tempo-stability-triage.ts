...
export const tempoStabilityTriage: ToolDefinition = {
	...
	description:
		"Distinguishes tempo drift, intentional rubato, and loss of pulse by checking monotonic drift and IOI correlation.",
	input_schema: {
		type: "object",
		properties: {
			bar_range: {
	...
	invoke: async (
		input: unknown,
		ctx?: import("../../loop/types").PhaseContext,
	): Promise<DiagnosisArtifact> => {
		if (!ctx)
			throw new Error(
				"tempo-stability-triage: ctx (PhaseContext with digest) is required",
			);
		const i = input as TempoSelectors;
		const ctx_r = await resolveMoleculeContext(
			ctx.digest as unknown as GroundedDigest,
			i.bar_range ?? null,
		);

		const muq_timing =
			ctx_r.bundle.muq_scores.length > 0
				? ctx_r.bundle.muq_scores.reduce((s, row) => s + row[1], 0) /
					ctx_r.bundle.muq_scores.length
		...
		const rangeLabel = i.bar_range
			? `bars ${i.bar_range[0]}-${i.bar_range[1]}`
			: "this session";
		...
		const neutral = DiagnosisArtifactSchema.parse({
			...
			one_sentence_finding:
				"Tempo is stable or within acceptable rubato range.",
			confidence: "low",
		...
		const perfNoteMap = new Map(
			ctx_r.bundle.midi_notes.map((n, perfIdx) => [perfIdx, n.onset_ms]),
		);
		...
		const r = (await computeIoiCorrelation.invoke({
		...
		const driftNotes = ctx_r.bundle.alignment
			...
			.map((a) => {
				...
				return {
					onset_ms: onset,
					expected_onset_ms: a.expected_onset_ms as number,
				};
			})
			.filter(
				(n): n is { onset_ms: number; expected_onset_ms: number } => n !== null,
			);

		const drift = (await computeOnsetDrift.invoke({
			notes: driftNotes,
		})) as OnsetDrift[];
		...
		const dominantFraction =
			Math.max(positiveCount, negativeCount) / drift.length;
		if (dominantFraction < 0.8) return neutral;

		const subtype: "slowing" | "rushing" =
			positiveCount > negativeCount ? "slowing" : "rushing";
		const finding =
			subtype === "slowing"
