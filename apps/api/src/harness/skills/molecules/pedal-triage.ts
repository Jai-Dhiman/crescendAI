...
export const pedalTriage: ToolDefinition = {
	name: "pedal-triage",
	description:
		"Distinguishes over-pedaling, under-pedaling, and pedal-timing issues by combining MuQ pedaling delta with AMT pedal CC overlap ratio.",
	input_schema: {
		type: "object",
		properties: {
			bar_range: {
	...
	invoke: async (
		...
		if (!ctx)
			throw new Error(
				"pedal-triage: ctx (PhaseContext with digest) is required",
			);
		...
		const muq_pedaling =
			ctx_r.bundle.muq_scores.length > 0
				? ctx_r.bundle.muq_scores.reduce((s, v) => s + v[2], 0) /
