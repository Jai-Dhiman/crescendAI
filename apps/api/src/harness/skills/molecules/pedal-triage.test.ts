...
test("pedalTriage: over-pedal (ratio>0.85, z<-1) with sufficient history returns issue/significant", async () => {
	...
	(ctx.digest as unknown as GroundedDigest).session_means.pedaling = [
		0.5, 0.6, 0.7,
	];
	const result = (await pedalTriage.invoke(
		selectors,
		ctx,
...
test("pedalTriage: insufficient session history (n<3) uses within-session baseline and returns artifact (not throw)", async () => {
	...
	(ctx.digest as unknown as GroundedDigest).session_means.pedaling = [0.5];
	(ctx.digest as unknown as GroundedDigest).within_session_means.pedaling = 0.6;
	const result = (await pedalTriage.invoke(
		selectors,
		ctx,
	)) as DiagnosisArtifact;
