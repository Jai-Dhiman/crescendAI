...
export const crossModalContradictionCheck: ToolDefinition = {
	...
	description:
		"Flags cases where MuQ dimension scores and AMT-derived structural features disagree on a passage. Articulation arm removed (requires score data). Checks: timing-drift, pedal-ratio, dynamics-range.",
	input_schema: {
		type: "object",
		properties: {
			bar_range: {
	...
	invoke: async (
		...
		if (!ctx)
			throw new Error(
				"cross-modal-contradiction-check: ctx (PhaseContext with digest) is required",
			);
		...
		async function dimZ(dimName: keyof typeof DIM): Promise<number> {
			const current = muq_scores
				? muq_scores[DIM[dimName]]
				: ctx_r.cohort[dimName].mean;
		...
		if (timingZ >= 0.5 && ctx_r.bundle.alignment.length >= 2) {
			...
			if (driftNotes.length >= 2) {
				...
				const meanDrift =
					drift.reduce((s, d) => s + d.drift_ms, 0) / drift.length;
				if (meanDrift > 80) {
					contradictions.push({
						dimension: "timing",
		...
		const winner = contradictions.reduce((best, c) =>
			c.delta > best.delta ? c : best,
		);
		const dims = [
			...new Set(contradictions.map((c) => c.dimension)),
		] as (keyof typeof DIM)[];
		return DiagnosisArtifactSchema.parse({
