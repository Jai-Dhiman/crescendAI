...
function makeCtx(
	...
	const digest: GroundedDigest = {
		...
		within_session_means:
			within_session_means as GroundedDigest["within_session_means"],
...
test("rubatoCoaching: monotonic drift (large, uncompensated) returns issue/significant", async () => {
	const ctx = makeCtx(3, 0.3, { monotonicDrift: true });
	...
	const result = (await rubatoCoaching.invoke(
		selectors,
		ctx,
...
test("rubatoCoaching: empty/sparse alignment returns neutral (not throw)", async () => {
	const ctx = makeCtx(3, 0.3, { emptyAlignment: true });
	(ctx.digest as unknown as GroundedDigest).session_means.timing = [
		0.5, 0.6, 0.7,
	];
	const selectors = {
...
test("rubatoCoaching: insufficient history uses within-session baseline and returns artifact (not throw)", async () => {
	const ctx = makeCtx(0, 0.3, { monotonicDrift: true });
	(ctx.digest as unknown as GroundedDigest).session_means.timing = [];
	(ctx.digest as unknown as GroundedDigest).within_session_means.timing = 0.6;
	const selectors = {
		bar_range: [40, 48] as [number, number],
		scope: "session" as const,
		evidence_refs: ["cache:muq:s1:c14"],
...
test("rubatoCoaching: missing ctx throws", async () => {
	...
	await expect(rubatoCoaching.invoke(selectors, undefined)).rejects.toThrow(
		"ctx",
	);
});
