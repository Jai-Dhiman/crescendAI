...
test('tempoStabilityTriage: z above neutral threshold returns neutral', async () => {
  const input = {
    ...
    muq_scores: [0.54, 0.55, 0.46, 0.54, 0.52, 0.51],
    midi_notes: [
      { pitch: 60, onset_ms: 0,   duration_ms: 400, velocity: 70, bar: 1 },
      { pitch: 62, onset_ms: 500, duration_ms: 400, velocity: 70, bar: 2 },
    ],
    alignment: [
      { perf_index: 0, score_index: 0, expected_onset_ms: 0,   bar: 1 },
      { perf_index: 1, score_index: 1, expected_onset_ms: 500, bar: 2 },
    ],
    session_means_timing: [0.55, 0.55, 0.55],
    piece_id: 'test-piece',
    now_ms: 1000,
  }
  const result = await tempoStabilityTriage.invoke(input) as DiagnosisArtifact
...
test('tempoStabilityTriage: insufficient session history throws', async () => {
  const input = {
    bar_range: [1, 2] as [number, number],
    scope: 'session' as const,
    evidence_refs: ['cache:muq:s1:c1'],
    muq_scores: [0.54, 0.38, 0.46, 0.54, 0.52, 0.51],
    midi_notes: [{ pitch: 60, onset_ms: 0, duration_ms: 400, velocity: 70, bar: 1 }],
    alignment: [{ perf_index: 0, score_index: 0, expected_onset_ms: 0, bar: 1 }],
    session_means_timing: [0.48, 0.54],
    piece_id: 'test-piece',
    now_ms: 1000,
  }
  await expect(tempoStabilityTriage.invoke(input)).rejects.toThrow('insufficient session history')
})
