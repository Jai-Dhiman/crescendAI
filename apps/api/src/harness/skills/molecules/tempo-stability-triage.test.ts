...
function makeCtx(
	...
	const midi_notes = Array.from({ length: N }, (_, k) => ({
		pitch: 60 + (k % 5),
		onset_ms: k * 500 + (opts?.monotonicDrift ? k * 40 : 0),
		duration_ms: 500,
		velocity: 70,
		bar: 1 + Math.floor(k / 4),
	}));
...
test("tempoStabilityTriage: monotonic positive drift returns issue/significant/slowing", async () => {
	...
	const selectors = {
		...
		evidence_refs: ["cache:muq:s1:c0"],
	};
	const result = (await tempoStabilityTriage.invoke(
		selectors,
		ctx,
...
test("tempoStabilityTriage: missing ctx throws", async () => {
	...
	await expect(
		tempoStabilityTriage.invoke(selectors, undefined),
	).rejects.toThrow("ctx");
});
