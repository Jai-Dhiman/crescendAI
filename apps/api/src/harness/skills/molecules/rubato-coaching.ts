...
export const rubatoCoaching: ToolDefinition = {
	...
	description:
		"Distinguishes intentional returned rubato from uncompensated drift using IOI correlation and net phrase-end drift.",
	input_schema: {
		type: "object",
		properties: {
			bar_range: {
	...
	invoke: async (
		...
		if (!ctx)
			throw new Error(
				"rubato-coaching: ctx (PhaseContext with digest) is required",
			);
		...
		const neutral = DiagnosisArtifactSchema.parse({
			...
			one_sentence_finding:
				"Timing is within baseline or rubato resolves cleanly.",
			confidence: "low",
		...
		const perfNoteMap = new Map(
			ctx_r.bundle.midi_notes.map((n, idx) => [idx, n.onset_ms]),
		);
		const alignMap = new Map(
			ctx_r.bundle.alignment.map((a) => [a.perf_index, a.expected_onset_ms]),
		);
		...
		const alignedNotes = ctx_r.bundle.midi_notes
			.map((n, idx) => ({
				onset_ms: n.onset_ms,
				expected_onset_ms: alignMap.get(idx),
			}))
			.filter(
				(n): n is { onset_ms: number; expected_onset_ms: number } =>
		...
		if (Math.abs(lastSigned) <= 50) {
			return DiagnosisArtifactSchema.parse({
				...
				evidence_refs: i.evidence_refs,
				one_sentence_finding:
					"The phrase stretched and came back; rubato is well-shaped.",
				confidence: "medium",
