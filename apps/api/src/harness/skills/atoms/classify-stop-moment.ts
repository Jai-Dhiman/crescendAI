...
const SCALER_MEAN = [0.545, 0.4848, 0.4594, 0.5369, 0.5188, 0.5064];
...
export const classifyStopMoment: ToolDefinition = {
	...
	input_schema: {
		...
		properties: {
			scores: {
				...
				maxItems: 6,
				description:
					"MuQ 6-dim quality scores: [dynamics, timing, pedaling, articulation, phrasing, interpretation]",
			},
		},
		required: ["scores"],
	},
	invoke: async (input: unknown): Promise<number> => {
		...
		if (!Array.isArray(scores) || scores.length !== 6) {
			throw new Error(
				"classify-stop-moment: scores must be an array of exactly 6 numbers",
			);
		}
		let logit = BIAS;
		for (let i = 0; i < 6; i++) {
