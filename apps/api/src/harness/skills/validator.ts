...
const ArtifactNameUnion = z.enum(
	ARTIFACT_NAMES as readonly [string, ...string[]],
);
...
const AtomFrontmatter = FrontmatterBase.extend({
	tier: z.literal("atom"),
}).refine((f) => f.depends_on.length === 0, {
	message: "atoms must have empty depends_on",
	path: ["depends_on"],
});

const MoleculeFrontmatter = FrontmatterBase.extend({
	tier: z.literal("molecule"),
});
...
export async function validateSkill(
	...
	if (!schema) {
		errors.push(
			`tier must be one of [atom, molecule, compound]; got ${String(data.tier)}`,
		);
	} else {
...
export async function validateCatalog(
	...
	for (const tier of ["molecule", "compound"] as const) {
		...
		const allowed =
			tier === "molecule"
				? tiers.atom
				: new Set([...tiers.molecule, ...tiers.atom]);
		let entries: string[] = [];
		try {
			entries = await readdir(join(rootDir, subdir));
		} catch {
			continue;
		}
		for (const entry of entries) {
			...
			for (const dep of dl as string[]) {
				if (!allowed.has(dep)) {
					errors.push(
						`${filePath}: depends_on entry "${dep}" does not resolve to an existing ${tier === "molecule" ? "atom" : "molecule or atom"} file`,
					);
				}
			}
		}
