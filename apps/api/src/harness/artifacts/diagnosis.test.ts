...
describe("DiagnosisArtifactSchema", () => {
	...
	test("rejects when primary_dimension is not in dimensions list", () => {
		const invalid = {
			...baseValid,
			primary_dimension: "timing" as const,
			dimensions: ["pedaling"] as const,
		};
		const result = DiagnosisArtifactSchema.safeParse(invalid);
		expect(result.success).toBe(false);
		expect(
			result.error?.issues.some((i) => i.message.includes("primary_dimension")),
		).toBe(true);
	});
	...
	test('rejects when bar_range is null but scope is not "session"', () => {
		...
		expect(result.success).toBe(false);
		expect(
			result.error?.issues.some((i) => i.message.includes("bar_range")),
		).toBe(true);
	});
	...
	test("accepts strength finding_type with minor severity", () => {
		const valid = {
			...baseValid,
			finding_type: "strength" as const,
