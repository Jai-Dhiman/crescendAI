...
app.post("/:id/accept", async (c) => {
	...
	try {
		const artifact = await segmentLoopsService.acceptSegmentLoop(
			c.var.db,
			id,
			studentId,
		);
		...
		if (err instanceof ValidationError)
			return c.json({ error: err.message, code: "invalid_state" }, 409);
		throw err;
	}
});
...
app.post("/:id/decline", async (c) => {
	...
	const id = c.req.param("id");
	try {
		const artifact = await segmentLoopsService.declineSegmentLoop(
			c.var.db,
			id,
			studentId,
		);
...
app.post("/:id/dismiss", async (c) => {
	...
	const id = c.req.param("id");
	try {
		const artifact = await segmentLoopsService.dismissSegmentLoop(
			c.var.db,
			id,
			studentId,
		);
