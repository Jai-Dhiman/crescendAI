...
const ASSET = embeddingsAsset as {
	dim: number;
	ids: string[];
	vectors: number[][];
};
...
describe("cosineSelectWithinDimension", () => {
	...
	test("returns the MOST similar drill within the bucket, not just the first", () => {
		...
		const timingIds = ASSET.ids.filter((x) =>
			MANIFEST[x]?.dimensions.includes("timing"),
		);
	...
	test("throws on a dimension/query dimensionality mismatch", () => {
		expect(() => cosineSelectWithinDimension([1, 2, 3], "timing")).toThrow(
			/dim/,
		);
	});
	...
	test("throws on a zero-magnitude query", () => {
		...
		expect(() => cosineSelectWithinDimension(zero, "timing")).toThrow(
			/zero magnitude/,
		);
	});
	...
	test("an unnormalized query (scaled) selects the same drill as normalized", () => {
		...
		expect(cosineSelectWithinDimension(scaled, "phrasing")?.primitiveId).toBe(
			id,
		);
	});
});
