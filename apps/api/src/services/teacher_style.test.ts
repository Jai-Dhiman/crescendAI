...
describe("teacher_style.evaluate", () => {
	it("evaluates arithmetic over signals", () => {
		expect(
			evaluate("1.5 * max_neg_dev + 0.3 * n_significant", SIGNALS),
		).toBeCloseTo(0.9, 6);
	});
	...
	it("rejects unknown signal names", () => {
		expect(() => evaluate("max_neg_dev + bogus", SIGNALS)).toThrow(
			/unknown signal/,
		);
	});
});
...
describe("teacher_style.evaluate conditionals", () => {
	it("returns then-branch when condition is true", () => {
		...
		expect(evaluate("1.5 if drilling_improved else 0", sig)).toBeCloseTo(
			1.5,
			6,
		);
	});
	...
	it("returns else-branch when condition is false", () => {
		...
		expect(evaluate("1.5 if drilling_improved else 0.5", sig)).toBeCloseTo(
			0.5,
			6,
		);
	});
	...
	it("evaluates the technical-corrective formula", () => {
		...
		const formula =
			"1.5 * max_neg_dev + 0.3 * n_significant - 0.5 * (1 if drilling_improved else 0)";
	...
	it("supports compound boolean: a < x and b < x", () => {
		...
		expect(
			evaluate("1 if max_neg_dev < 0.1 and max_pos_dev < 0.1 else 0", sig),
		).toBeCloseTo(1, 6);
	});
	...
	it("evaluate handles unary minus", () => {
		...
		expect(
			evaluate("max_neg_dev - -0.5", { ...SIGNALS, max_neg_dev: 1.0 }),
		).toBeCloseTo(1.5);
	});
});
...
describe("teacher_style.selectClusters parity fixtures", () => {
	for (const f of fixtures) {
		it(`fixture ${f.name}: primary contains ${f.expected_primary_substring}`, () => {
			...
			expect(sel.primary.name.toLowerCase()).toContain(
				f.expected_primary_substring.toLowerCase(),
			);
			expect(sel.secondary.name.toLowerCase()).toContain(
				f.expected_secondary_substring.toLowerCase(),
			);
		});
	}
