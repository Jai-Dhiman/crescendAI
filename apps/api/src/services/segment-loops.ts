...
import {
	SEGMENT_LOOP_STATUSES,
	segmentLoops,
} from "../db/schema/segment-loops";
...
const TERMINAL_STATUSES: SegmentLoopStatus[] = [
	"completed",
	"dismissed",
	"superseded",
];
...
function rowToArtifact(
	row: typeof segmentLoops.$inferSelect,
): SegmentLoopArtifact {
	if (!SEGMENT_LOOP_STATUSES.includes(row.status as SegmentLoopStatus)) {
		throw new ValidationError(
			`invalid segment_loop status in DB: '${row.status}'`,
		);
	}
	if (
		row.dimension !== null &&
		!(DIMS_6 as readonly string[]).includes(row.dimension)
	) {
		throw new ValidationError(
			`invalid segment_loop dimension in DB: '${row.dimension}'`,
		);
	}
	return {
		kind: "segment_loop",
...
async function assertOwnerAndLoad(
	...
	if (!row || row.studentId !== studentId)
		throw new NotFoundError("segment_loop", id);
	return row;
}
...
function assertNotTerminal(status: string, id: string): void {
	if (TERMINAL_STATUSES.includes(status as SegmentLoopStatus)) {
		throw new ValidationError(
			`segment_loop ${id} is in terminal state '${status}'`,
		);
	}
}
...
export async function acceptSegmentLoop(
	...
	if (row.status !== "pending") {
		throw new ValidationError(
			`accept requires status 'pending', got '${row.status}'`,
		);
	}
...
export async function declineSegmentLoop(
	...
	if (row.status !== "pending") {
		throw new ValidationError(
			`decline requires status 'pending', got '${row.status}'`,
		);
	}
	const [updated] = await db
		.update(segmentLoops)
...
export async function incrementAttempts(
	...
	if (row.status !== "active") {
		throw new ValidationError(
			`incrementAttempts requires status 'active', got '${row.status}'`,
		);
	}
...
export function toLoopComponent(
	artifact: SegmentLoopArtifact,
): InlineComponent {
	return {
		type: "segment_loop",
		config: {
