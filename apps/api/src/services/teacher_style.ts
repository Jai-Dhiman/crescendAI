...
function expr(c: Cursor, s: Signals): number {
	...
	if (nxt && nxt.kind === "ident" && nxt.text === "if") {
		...
		const kw = c.peek();
		if (!kw || kw.text !== "else")
			throw new Error("expected 'else' in conditional");
		c.take();
...
function boolNot(c: Cursor, s: Signals): boolean {
	const nxt = c.peek();
	if (nxt && nxt.kind === "ident" && nxt.text === "not") {
		c.take();
		return !cmp(c, s);
	}
	return cmp(c, s);
...
function factor(c: Cursor, s: Signals): number {
	...
	if (nxt.kind === "number") {
		c.take();
		return parseFloat(nxt.text);
	}
	if (nxt.kind === "ident") {
		if (!ALLOWED_SIGNALS.has(nxt.text))
			throw new Error(`unknown signal: ${nxt.text}`);
		c.take();
		const v = s[nxt.text];
...
function findCluster(substring: string): Cluster {
	const clusters = (playbookRaw as unknown as Playbook).teaching_playbook
		.clusters;
	const subNorm = normalizeForMatch(substring);
	const found = clusters.find((c) =>
		normalizeForMatch(cleanFormula(c.name)).includes(subNorm),
	);
...
export function formatTeacherVoiceBlocks(selection: ClusterSelection): string {
	...
	const pReg = p.raw.language_patterns?.register
		? cleanFormula(p.raw.language_patterns.register)
		: "";
	const pTone = p.raw.language_patterns?.tone
		? cleanFormula(p.raw.language_patterns.tone)
		: "";
	...
	const sWhen = (s.raw.when_to_use ?? [])
		.map((w) => cleanFormula(w))
		.join("; ");
...
export function deriveSignals(
	...
	const drilling = Array.isArray(drillingRecords)
		? (drillingRecords as DrillingRecord[])
		: [];

	const devs = moments
		.map((m) => Number(m.deviation_from_mean))
	...
	const drilling_improved =
		drilling_present &&
		Number(drilling[drilling.length - 1].final_score) >
			Number(drilling[0].first_score) + 0.15;

	const duration_min = sessionDurationMs / 60_000;

	let mode_count = 1;
	if (Array.isArray(practicePattern))
		mode_count = new Set(practicePattern).size;
