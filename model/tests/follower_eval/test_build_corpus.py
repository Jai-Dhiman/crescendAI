...
def _write_candidates(root: Path, piece: str, recs: list[dict]) -> None:
    ...
    (d / "candidates.yaml").write_text(
        yaml.safe_dump({"piece": piece, "recordings": recs})
    )
...
def test_approved_videos_filters_unapproved(tmp_path: Path):
    ...
    _write_candidates(
        tmp_path, "bach_invention_1", [{"video_id": "d", "approved": True}]
    )
...
def test_build_one_skips_existing_bundle(tmp_path: Path):
    ...
    out = bc.build_one(
        "p", {"video_id": "vid"}, tmp_path / "audio", bundle, _fail_transcribe
    )
