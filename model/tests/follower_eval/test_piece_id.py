...
def _cand(sid, cov, conf, votes=0, source="ngram") -> pid.Candidate:
    return pid.Candidate(
        score_id=sid, source=source, ngram_votes=votes, coverage=cov, confidence=conf
    )
...
def test_ngram_shortlist_votes_and_rank():
    ...
    perf = [
        ...
        _pn(2, 64),
        _pn(3, 65),
    ]  # trigrams 60,62,64 and 62,64,65
...
def test_decide_accepts_clear_winner():
    ...
    cands = sorted(
        [_cand("right", 0.62, 0.84), _cand("wrong", 0.51, 0.06)],
        key=pid._decision_score,
        reverse=True,
    )
...
def test_decide_confirms_label_over_high_coverage_wrong():
    ...
    cands = sorted(
        [
            _cand("bach.prelude.bwv_846", 0.99, 0.97, source="label"),
...
def test_decide_abstains_on_low_confidence():
    ...
    cands = sorted(
        [_cand("a", 0.70, 0.30), _cand("b", 0.66, 0.28)],
        key=pid._decision_score,
        reverse=True,
    )
...
def test_decide_abstains_on_thin_margin():
    ...
    cands = sorted(
        [_cand("a", 0.80, 0.80), _cand("b", 0.78, 0.80)],
        key=pid._decision_score,
        reverse=True,
    )
    assert pid.decide(cands) == "ABSTAIN"
