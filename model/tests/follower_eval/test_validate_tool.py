...
def test_load_piece_id_map_drops_abstains(tmp_path):
    p = _write_piece_id(tmp_path, [
        {"piece_folder": "fantaisie_impromptu", "video_id": "x", "decision": "chopin.etudes_op_25.5"},
...
def test_resolve_prefers_identified_score_over_label():
    sid, src = vt.resolve_score_id("fantaisie_impromptu", "x",
                                   {"fantaisie_impromptu/x": "chopin.etudes_op_25.5"})
...
def test_list_clips_attaches_resolved_score_and_relabel_flag(tmp_path):
    ...
    subset.write_text(json.dumps({"clips": [{"piece": "fantaisie_impromptu",
                                             "video_id": "x", "v1_confidence": 0.1}]}))
    clips = vt.list_clips(subset, bundles, use_all=False, pieces=None,
...
def test_list_clips_confirmed_label_is_not_relabeled(tmp_path):
    ...
    subset = tmp_path / "subset.json"
    subset.write_text(json.dumps({"clips": [{"piece": "bach_prelude_c_wtc1",
                                             "video_id": "w", "v1_confidence": 0.9}]}))
    clips = vt.list_clips(subset, bundles, use_all=False, pieces=None,
...
def test_view_cache_key_separates_scores(tmp_path):
    ...
    a = vt._view_cache_path(tmp_path, "fantaisie_impromptu", "x", "chopin.fantaisie_impromptu")
    b = vt._view_cache_path(tmp_path, "fantaisie_impromptu", "x", "chopin.etudes_op_25.5")
    assert a != b


def test_generate_html_surfaces_unverified_scores(tmp_path):
    clips = [{"piece": "fur_elise", "video_id": "y", "title": None, "v1_confidence": 0.2,
              "existing": False, "score_id": "beethoven.fur_elise", "score_source": "label",
              "relabeled": False}]
