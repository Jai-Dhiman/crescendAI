...
def load_teacher_model(checkpoint_path: Path, device: str = 'cuda') -> Tuple[torch.nn.Module, dict]:
    """
...
def extract_features_from_midi(
    ...
    try:
        import pretty_midi
        from data.score_alignment import ScoreAlignmentFeatureExtractor, NUM_NOTE_FEATURES
        ...
        note_locations = create_note_locations(features.get('note_locations', {}), num_notes, max_notes)
...
def extract_midi_only_features(midi: 'pretty_midi.PrettyMIDI', max_notes: int) -> Dict:
    ...
    global_features = np.array([
        ...
        midi.estimate_tempo() if hasattr(midi, 'estimate_tempo') else 120.0,  # tempo
    ], dtype=np.float32)
...
def create_note_locations(locations: Dict, num_notes: int, max_notes: int) -> Dict[str, torch.Tensor]:
...
def pseudo_label_maestro(
    ...
    if teacher_r2 < 0.15:
        ...
        print("Pseudo-labels may not be reliable. Consider training a better teacher first.")
    ...
    print(f"Reliable samples (conf >= {confidence_threshold}): {reliable_count} ({100*reliable_count/len(pseudo_labels):.1f}%)")
