...
def check_hierarchical_aggregation(model, dataloader, device):
    ...
    print(f"Beat range: [{note_locations['beat'].min().item()}, {note_locations['beat'].max().item()}]")
    print(f"Measure range: [{note_locations['measure'].min().item()}, {note_locations['measure'].max().item()}]")
...
def check_attention_collapse(model, dataloader, device):
    ...
    with torch.no_grad():
        ...
        entropy = -(final_softmax * (final_softmax + 1e-10).log()).sum(dim=1).mean().item()
