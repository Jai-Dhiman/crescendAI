...
def check_annotation_files(
    ...
    for split, path in files.items():
        ...
        is_local = '/tmp/' in str(path_obj) or '/content/' in str(path_obj) and '/drive/' not in str(path_obj)
...
def check_data_loading(
    ...
    for i in range(num_to_test):
        ...
        try:
            ...
            if load_time > 2.0:
                print(f"  ⚠ Sample {i+1}: SLOW ({load_time:.2f}s) - check data location!")
            else:
                print(f"  ✓ Sample {i+1}: {load_time:.3f}s ({len(audio):,} samples, {len(tokens) if midi_path else 0} MIDI tokens)")
    ...
    print(f"\n  Summary:")
    print(f"    Audio loaded: {results['audio_success']}/{num_to_test} ({success_rate*100:.0f}%)")
...
def check_dataloader_speed(
    ...
    print(f"  Estimated epoch time: {estimated_epoch_time/60:.1f} min ({batches_per_epoch} batches)")
...
def check_gpu() -> Tuple[bool, dict]:
    ...
    try:
        ...
        if gpu_memory_gb < required_memory:
            print(f"  ⚠ Warning: {gpu_memory_gb:.1f} GB < {required_memory} GB recommended")
