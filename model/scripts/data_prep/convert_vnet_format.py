    ...
    for key in VNET_INPUT_KEYS:
        ...
        if dim == 1:
            ...
            for i, vec in enumerate(feat_values):
                input_features[i, feature_idx:feature_idx + dim] = np.array(vec[:dim], dtype=np.float32)
...
def compute_normalization_stats(all_features: list) -> dict:
    ...
    print(f"  Computing stats from {all_data.shape[0]} notes across {len(all_features)} samples")
...
def main():
    ...
    label_file = Path("data/raw/PercePiano/label_2round_mean_reg_19_with0_rm_highstd0.json")
