...
def update_annotation_paths(
    annotations_dir: Path, output_annotations_dir: Path, base_name: str = ""
) -> None:
    """
...
def calculate_sizes(segments_dir: Path) -> dict:
    ...
    if annotations_dir.exists():
        sizes["annotations"] = sum(
            f.stat().st_size for f in annotations_dir.glob("*.jsonl")
        )
...
def package_dataset(
    ...
    print(f"\nSource sizes:")
    print(
        f"  Audio: {sizes.get('audio', 0) / 1e9:.2f} GB ({sizes.get('audio_count', 0)} files)"
    )
    print(
        f"  MIDI: {sizes.get('midi', 0) / 1e6:.2f} MB ({sizes.get('midi_count', 0)} files)"
    )
    print(
        f"  Annotations: {sizes.get('annotations', 0) / 1e3:.1f} KB ({sizes.get('annotation_count', 0)} files)"
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        update_annotation_paths(segments_dir / "annotations", tmp_annotations)
...
def main():
    ...
    output_path = (
        ...
        if args.output_path
        else segments_dir / "maestro_with_variance.tar.gz"
    )
