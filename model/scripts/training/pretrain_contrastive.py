...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Contrastive pre-training for MERT-MIDIBert alignment"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/tmp/checkpoints/contrastive",
    ...
    parser.add_argument(
        "--shared_dim", type=int, default=512, help="Shared projection space dimension"
    )
    parser.add_argument(
        "--freeze_encoders",
        action="store_true",
...
def main():
    ...
    if best_alignment >= args.alignment_target:
        ...
        print(
            "-> Consider: More epochs, different temperature, or architecture changes"
        )
        gate_passed = False
    ...
    results = {
        ...
        "args": vars(args),
        "best_alignment_score": float(best_alignment)
