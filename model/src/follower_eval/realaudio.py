...
SCORE_FILENAME_BY_PIECE = {
    piece: path.name for piece, path in DEFAULT_SCORE_BY_PIECE.items()
}
...
def load_bundle_notes(bundle_path: Path) -> list[PerfNote]:
    ...
    if not notes:
        raise RealAudioEvalError(
            f"{bundle_path.name}: bundle has no 'notes' (not transcribed?)"
        )
    pn = [
        PerfNote(
            onset=float(n["onset"]),
...
def load_score(score_path: Path) -> tuple[list[ScoreNote], tuple[int, ...], float]:
    ...
    score_notes = [
        ScoreNote(pitch=int(s["pitch"]), position=float(s["onset_sec"]))
        for s in score_na
    ]
    ...
    span = (
        float(score_na["onset_sec"].max() - score_na["onset_sec"].min())
        if len(score_na)
        else 0.0
    )
...
def _mono_stats(score_positions: list[float]) -> tuple[int, float, float]:
    ...
    if len(score_positions) < 2:
        return 0, 0.0, 0.0
    deltas = [
        score_positions[i] - score_positions[i - 1]
        for i in range(1, len(score_positions))
    ]
...
def run_clip(
    ...
    if len(ms) >= 4 and len(confs) == len(ms):
        step_ok = [1.0] + [
            1.0 if positions[k] - positions[k - 1] >= -MONO_TOL_SEC else 0.0
    ...
    return ClipProxies(
        ...
        score_span_frac=round((smax - smin) / score_span_sec, 4)
        if score_span_sec
        else 0.0,
...
def _aggregate(clips: list[ClipProxies], group: bool = True) -> dict:
    ...
    for key, group_clips in buckets.items():
        confs = [
            c.confidence_median for c in group_clips if c.confidence_median is not None
        ]
        cals = [
            c.conf_vs_monotone_spearman
        ...
        out[key] = {
            ...
            "median_coverage": round(
                statistics.median(c.coverage for c in group_clips), 4
            ),
            "median_span_frac": round(
                statistics.median(c.score_span_frac for c in group_clips), 4
            ),
            ...
            "median_backward_frac": round(
                statistics.median(c.backward_frac for c in group_clips), 4
            ),
...
def _format_report(result: dict) -> str:
    ...
    if result["unknown_piece_dirs"]:
        lines.append(
            f"unknown piece dirs (no score, skipped): {result['unknown_piece_dirs']}"
        )
    lines.append("")
    hdr = (
        f"{'piece':<24}{'clip':<18}{'cov':>7}{'span':>7}{'conf':>7}"
    ...
    for c in result["clips"]:
        conf = (
            f"{c.confidence_median:.2f}" if c.confidence_median is not None else "  n/a"
        )
        cal = (
            f"{c.conf_vs_monotone_spearman:+.2f}"
            if c.conf_vs_monotone_spearman is not None
            else " n/a"
        )
        lines.append(
            f"{c.piece:<24}{c.bundle[:17]:<18}{c.coverage:>7.2f}"
    ...
    for piece, agg in sorted(result["per_piece"].items()):
        conf = (
            f"{agg['median_confidence']:.2f}"
            if agg["median_confidence"] is not None
            else "n/a"
        )
        cal = (
            f"{agg['median_calibration']:+.2f}"
            if agg["median_calibration"] is not None
            else "n/a"
        )
        lines.append(
            f"  {piece:<24} n={agg['n_clips']:<3} cov={agg['median_coverage']:.2f} "
    ...
    if result["overall"]:
        ...
        lines.append("")
        lines.append(
            f"OVERALL MEDIANS  cov={o['median_coverage']:.2f} "
            f"span={o['median_span_frac']:.2f} "
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Real-audio follower eval -- proxy track (#133)"
    )
    ap.add_argument(
        "--bundles-root",
        type=Path,
        default=Path("data/evals/claim_bundles"),
