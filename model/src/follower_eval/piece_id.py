...
from follower_eval.realaudio import SCORE_FILENAME_BY_PIECE, load_bundle_notes, load_score
...
def _verify(window: list[PerfNote], score_id: str, scores_root: Path) -> tuple[float, float | None]:
    ...
    est = follow_hmm(window, score_notes, TUNED_HMM_PARAMS, bar_boundaries=bar_boundaries)
...
def identify(piece_folder: str, video_id: str, bundles_root: Path, scores_root: Path,
             idx: dict, k: int = NGRAM_SHORTLIST_K, window_sec: float = WINDOW_SEC) -> PieceIdResult:
    ...
    for sid in shortlist:
        ...
        src = "ngram+label" if (sid in ng and sid == label_sid) else ("label" if sid == label_sid else "ngram")
        cands.append(Candidate(score_id=sid, source=src, ngram_votes=ng.get(sid, 0),
                               coverage=round(cov, 4), confidence=round(conf, 4) if conf is not None else None))

    cands.sort(key=_decision_score, reverse=True)
    decision = decide(cands)
    return PieceIdResult(
        piece_folder=piece_folder, video_id=video_id, decision=decision,
        label_agrees=(decision == label_sid), n_window_notes=len(window),
...
def decide(cands: list[Candidate]) -> str:
    ...
    if (best.coverage >= ACCEPT_COVERAGE and (best.confidence or 0.0) >= ACCEPT_CONFIDENCE
...
def _format(results: list[PieceIdResult]) -> str:
    L = ["=" * 84,
         "REAL-AUDIO FOLLOWER EVAL (#133) -- PIECE-ID -- identify the score actually played",
         "=" * 84]
    ...
    L.append(f"clips: {len(results)}   label-confirmed: {agree}   RE-LABELED: {relabel}   abstain: {abstain}")
    L.append("")
    for r in results:
        L.append(f"{r.piece_folder}/{r.video_id}")
        L.append(f"  -> decision: {r.decision}"
                 + ("  (== label, confirmed)" if r.label_agrees else (f"  (RE-LABELED, was {r.piece_folder})" if r.decision != 'ABSTAIN' else "  (label rejected -> abstain)")))
        for c in r.candidates[:4]:
            conf = f"{c.confidence:.2f}" if c.confidence is not None else " n/a"
            L.append(f"       cov={c.coverage:.2f} conf={conf} votes={c.ngram_votes:<4} [{c.source}] {c.score_id}")
    return "\n".join(L)


def main() -> None:
    ap = argparse.ArgumentParser(description="Piece-ID stage -- identify the score actually played (#133)")
    ap.add_argument("--clips", nargs="+", required=True, help="piece_folder/video_id pairs")
    ...
    ap.add_argument("--k", type=int, default=NGRAM_SHORTLIST_K, help="ngram shortlist size")
    ap.add_argument("--window-sec", type=float, default=WINDOW_SEC, help="verify-window length")
