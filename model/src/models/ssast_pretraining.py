...
class SSASTPreTrainingModel(nn.Module):
    ...
    def __call__(self, 
        ...
        return {
            ...
            # Expose encoder (sequence) embeddings for SSL objectives (e.g., InfoNCE)
...
def create_ssast_train_state(model: SSASTPreTrainingModel,
                            ...
                            accumulate_steps: int = 1) -> train_state.TrainState:
    ...
    # AdamW optimizer with weight decay + optional gradient accumulation
    base_tx = optax.chain(
    ...
    if int(accumulate_steps) > 1:
        tx = optax.chain(
            base_tx,
            optax.apply_every_k(k=int(accumulate_steps))
        )
    else:
        tx = base_tx
