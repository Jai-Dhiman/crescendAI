...
class MAESTRODataset:
    ...
    def _extract_mel_spectrogram(
        ...
        """Extract mel-spectrogram from audio file using centralized preprocessing.

        Returns time-major dB spectrogram clipped to [-80, 0].
        """
        try:
            # Load audio (mono, target_sr)
            y = load_audio_mono_22050(audio_path, target_sr=self.target_sr)

            # Compute full-length mel-dB (time-major)
            ...
            return log_mel_t  # Shape: (time_frames, n_mels), values in [-80, 0]
    ...
    def get_data_iterator(
        ...
        Args:
            ...
            unique_per_file: If True, at most one segment per file is used per pass
        Returns:
            Iterator yielding batches of spectrograms [B, T=segment_length, F=n_mels]
        """
        ...
        def data_generator():
            while True:
                ...
                for idx in indices:
                    ...
                    if unique_per_file:
                        # Select a single segment per file to avoid false negatives in InfoNCE
                        # Choose a random segment index deterministically from numpy RNG
                        seg_idx = np.random.randint(0, len(segments))
                        batch.append(segments[seg_idx])
                    else:
                        for segment in segments:
                            batch.append(segment)

                    if len(batch) >= batch_size:
                        # Trim to batch_size to avoid overshoot and yield
                ...
                if batch and len(batch) > 0:
                    # Pad to batch_size if needed by repeating first sample
