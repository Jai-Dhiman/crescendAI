...
class MERTMultiLayerExtractor:
    """MERT-330M extractor that concatenates specific layers.

    Instead of averaging across a layer range, this extractor concatenates
    embeddings from specific layers (e.g., [6, 9, 12]) to create a multi-
    ...
    def __init__(
        ...
        print(f"Loading MERT-v1-330M (concat layers {layers}) on {self.device}...")
        self.processor = AutoProcessor.from_pretrained(
            "m-a-p/MERT-v1-330M", trust_remote_code=True
        )
        self.model = AutoModel.from_pretrained(
        ...
        self.model.eval()
        print(f"Model loaded. Output dim: {self.model.config.hidden_size * len(layers)}")
    ...
    def extract_from_file(self, audio_path: Path, use_cache: bool = True) -> torch.Tensor:
        ...
        outputs = self.model(**inputs)
        # Concatenate specific layers instead of averaging
        layer_embeddings = [outputs.hidden_states[l] for l in self.layers]
        embeddings = torch.cat(layer_embeddings, dim=-1).squeeze(0).cpu()
...
def extract_mert_multilayer_concat(
    layers: List[int],
    audio_dir: Path,
    cache_dir: Path,
    keys: List[str],
) -> int:
    """Extract MERT embeddings by concatenating specific layers.

    Args:
        layers: List of layer indices to concatenate (e.g., [6, 9, 12])
    ...
    print(f"Extracting {len(to_extract)} embeddings (concat layers [{layer_str}])...")
    extractor = MERTMultiLayerExtractor(layers, cache_dir)

    for key in tqdm(to_extract, desc=f"MERT L[{layer_str}]"):
        audio_path = Path(audio_dir) / f"{key}.wav"
        if audio_path.exists():
            extractor.extract_from_file(audio_path)
