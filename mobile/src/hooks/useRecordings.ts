...
export function useRecordings() {
  ...
  const createRecordingMutation = useMutation({
    mutationFn: (recordingData: Omit<Recording, 'id' | 'createdAt' | 'updatedAt'>) =>
      apiClient.createRecording(recordingData),
    onSuccess: (response) => {
      addRecording(response.data);
      queryClient.invalidateQueries({ queryKey: queryKeys.recordingsList(user?.id || '') });
    },
  });

  // Update recording mutation
  const updateRecordingMutation = useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: Partial<Recording> }) =>
      apiClient.updateRecording(id, updates),
    onSuccess: (response, variables) => {
      updateRecording(variables.id, response.data);
      queryClient.setQueryData(queryKeys.recording(variables.id), response.data);
      queryClient.invalidateQueries({ queryKey: queryKeys.recordingsList(user?.id || '') });
    },
  });
  ...
  const uploadRecordingMutation = useMutation({
    mutationFn: ({ recordingId, audioFile }: { recordingId: string; audioFile: File | Blob }) =>
  ...
  const createRecording = (recordingData: Omit<Recording, 'id' | 'createdAt' | 'updatedAt'>) => {
