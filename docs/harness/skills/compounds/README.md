...
## Compound catalog
- `session-synthesis` -- triggered_by: `OnSessionEnd`. Runs 7 diagnosis molecules in parallel, calls `prioritize-diagnoses`, and writes one SynthesisArtifact (synthesis_scope=session). Exercise prescriptions follow the separate `ExerciseRoutingDecision` contract in `docs/apps/04-exercises.md`. Reads live-practice-companion's stop_moment DiagnosisArtifacts as inputs (Option B compound-overlap policy).
