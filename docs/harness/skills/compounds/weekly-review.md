...
PHASE 5 (exercise routing):
7. Route any weekly prescription through the `ExerciseRoutingDecision` contract; this compound does not call a proposal molecule.

PHASE 6 (single write):
8. Compose SynthesisArtifact: session_id (synthetic, e.g., 'weekly:stu_42:2026-W17'), synthesis_scope='weekly', strengths (top 2 across week), focus_areas (top 3), prescribed_exercise (at most one decision from PHASE 5), dominant_dimension, recurring_pattern (mandatory non-null), next_session_focus, diagnosis_refs (all collected), headline derived LAST in week-summary teacher voice (300-500 chars).
