...
**Tech Stack:** Claude API or Cloudflare Workers AI (Llama 3.3 70B), structured prompt template
...
**Primary:** Claude (via Anthropic API) -- best at following nuanced persona instructions, natural tone.

**Fallback:** Cloudflare Workers AI (Llama 3.3 70B) -- lower latency from Workers, free on Cloudflare, but less refined output.

**Latency budget:** LLM call should complete in <2 seconds. With structured input (not long context), this is achievable with either model.
...
- API integration (Claude or Workers AI)
...
1. Claude API vs. Workers AI for production? Claude is better quality but adds latency (network hop) and cost. Workers AI is free and co-located but lower quality.
