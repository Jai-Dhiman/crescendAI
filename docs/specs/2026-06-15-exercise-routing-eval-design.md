...
### The cross-package change: V6 gate + surface prescribed_exercise in eval_context

**Problem:** `eval_context` (attached to the synthesis WS message when `isEvalSession=true`) currently carries `scored_chunks`, `teaching_moments`, `baselines`, etc. but NOT `prescribed_exercise` from `SynthesisArtifact`. Worse, V6 (which produces `SynthesisArtifact`) is explicitly gated out for ALL eval sessions at `session-brain.ts` line 1719:

```typescript
if (this.env.HARNESS_V6_ENABLED === "true" && !state.isEvalSession) {
```

This means `artifact` never exists for eval sessions — the `#22` ASCF baseline was measured on the legacy `teacherSynthesize` path, and that must be preserved. However, the new routing eval uses `chunk_ready` with real audio (full accumulator state), so V6 MUST run to produce `SynthesisArtifact.prescribed_exercise`.

**Fix:** Distinguish the two eval paths by tracking whether a session received any `chunk_ready` message (real inference) vs only `eval_chunk` messages (pre-computed scores). Add `receivedRealInferenceChunk: boolean` (default `false`) to `SessionState` schema. `handleChunkReady` sets it `true` inside the existing `blockConcurrencyWhile` lock. Change the V6 gate to:

```typescript
if (this.env.HARNESS_V6_ENABLED === "true" && (!state.isEvalSession || state.receivedRealInferenceChunk)) {
```

This preserves all three behavioral requirements:
- Production sessions (`isEvalSession=false`): always V6 (unchanged).
- `#22` eval sessions (only `eval_chunk` messages): `receivedRealInferenceChunk=false` → legacy path (unchanged; baseline preserved).
- New routing eval sessions (`chunk_ready` with real audio): `receivedRealInferenceChunk=true` → V6 runs.

Inside the V6 block, the `wsPayloadWithEval` assembly is updated to inject `prescribed_exercise` and the full `artifact` into `eval_context` (both are in scope as `artifact` is the validated `SynthesisArtifact`):
...
The `SynthesisResult` dataclass in `apps/evals/shared/pipeline_client.py` is extended with a `prescribed_exercise: dict | None` field parsed from `eval_context`.
...
| File | Change | Type |
|------|--------|------|
| `apps/api/src/do/session-brain.schema.ts` | Add `receivedRealInferenceChunk: z.boolean().default(false)` to `sessionStateSchema`; add field to `createInitialState` | Modify |
| `apps/api/src/do/session-brain.ts` | Set `receivedRealInferenceChunk=true` in `handleChunkReady`'s `blockConcurrencyWhile`; change V6 gate to `(!state.isEvalSession \|\| state.receivedRealInferenceChunk)`; inject `prescribed_exercise` + `artifact` into `eval_context` in `wsPayloadWithEval` (V6 block only) | Modify |
