...
4. **CPT Probe:** Continual pretraining on Qwen3.5-7B with 20M tokens confirms knowledge injection (post-CPT score >= base_score + 15pp on corpus-derived held-out probe). Cost: ~$15, 2 hours.
...
- Checkpointing: Every 10% of training. Run corpus-derived probe at each checkpoint.
- Compute: ~8-16 hours on 4xH100, ~$400-700. Budget 2-3x for restarts.
- Test staged vs monolithic approach in 7B probe (Gate 4)

**CPT Gate:** Post-CPT perplexity on held-out pedagogy corpus must decrease. Perplexity on 1M-token SlimPajama general slice must not increase by more than 5%. Corpus-derived probe score must be >= base_model_score + 15pp (see Probe Design below). If gate fails: reduce LR to 5e-6 and re-run. If still failing, increase SlimPajama ratio to 30%. If still failing after 2 attempts, re-evaluate base model choice.

**Probe Design (corpus-derived, not model-generated):** The probe must be built from held-out corpus documents, not from model-generated questions. Rationale: questions generated by the base model (e.g., Sonnet 4.6) test general music knowledge already in pretraining — Sonnet scores 99/100 on such questions. The meaningful signal is corpus-specific knowledge (named-teacher techniques, specific masterclass advice, specific piece commentary) that the base model does not already know.

Procedure:
1. Split the collected corpus 80/20 by document before CPT begins (frozen split — no held-out documents ever touch training).
2. Run a question generator on the 20% held-out documents: extract 3-5 MCQ questions per document (specific factual claims, plausible distractors). Use Workers AI for generation (~$0.15 per 100 documents).
3. Run base Qwen 7B (no CPT) on all generated questions. Discard questions where the base model already answers correctly — those are not measuring corpus knowledge.
4. The surviving questions (expected ~30-40% of generated set) are the probe. Target: 200-400 probe questions from 100+ held-out documents.
5. Gate: post-CPT Qwen 7B score on probe >= base_score + 15pp. The old `data/domain_probe.json` (100 general MCQ) is retained as a catastrophic-forgetting regression check, not as the gate instrument.

Scripts: `teacher_model/corpus_split.py` (document-level train/test split), `teacher_model/probe_generator.py` (question generation from held-out docs).
...
1. **Corpus-Derived Probe** (`teacher_model/probe_generator.py` + `corpus_split.py`): MCQ questions generated from held-out corpus documents (20% split, frozen before CPT). Base Qwen 7B establishes the baseline; only questions the base model fails are used. Gate: post-CPT score >= base_score + 15pp. The existing `data/domain_probe.json` (100 general MCQ, Sonnet scores 99/100) is retained as a catastrophic-forgetting regression check only.
...
- [ ] **Gate 4 (CPT probe):** After corpus 80/20 split is frozen, generate probe questions from held-out docs (`probe_generator.py`). Run base Qwen 7B to establish baseline. Run 7B CPT on 20M tokens, re-run probe. Need post-CPT >= base_score + 15pp. The old general MCQ probe (`domain_knowledge_probe.py`, Sonnet=99/100) is a regression check only, not the gate.
