...
**Goal:** Build the iOS audio capture layer that records continuously during a practice session and produces 15-second audio chunks and runs on-device MuQ inference via Core ML.

**Architecture:** Native iOS app using AVAudioEngine for real-time audio capture with a rolling buffer. Audio is chunked on a timer. On-device inference via Core ML replaces cloud upload for scoring. Background audio mode keeps recording when screen is off.
...
**Task 4: Integrate Core ML MuQ inference**

- After each 15s chunk is extracted from the ring buffer, run it through the Core ML MuQ model
- Store the 6-dimension scores in the on-device session buffer (SwiftData)
- Handle model loading, memory management, and inference errors
...
1. Ring buffer size: 5 minutes enough? Could a student play for longer without asking "how was that?"
2. AAC encoding quality: 64kbps enough for MuQ, or does it need higher bitrate?
3. Core ML memory management: Can MuQ stay loaded in memory during the entire session, or should it be loaded per-chunk to reduce memory pressure?
