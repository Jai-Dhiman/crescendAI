...
**Phase-1a open-set gate ported to WASM (#26, additive — runtime UNCHANGED):** The Phase-0-certified open-set piece-ID gate is now ported to Rust/WASM under `apps/api/src/wasm/piece-identify/` (`chroma.rs` for 12-bin chroma recall, `gate.rs` for the pitch-only chord-set Jaccard elastic-DTW margin gate @ threshold 0.0935, `identify.rs` for the chroma-recall→gate flow), exported as `identify_piece` and wrapped by `wasm-bridge.ts` as `identifyPiece`. A v2 `piece_index.json` artifact generator (`just fingerprint`) and a Rust↔Python port-fidelity harness (`parity_test.rs` vs committed golden `parity_fixtures.json`, 1e-4 tolerance, TA 14/16, OOD 0) land alongside. **This is purely additive: the SessionBrain DO is NOT wired to it and still runs the legacy fingerprint/v1 path (`text_match.rs` + chroma-DTW above) — runtime behavior is unchanged by this PR.** The DO cutover (cross-chunk note-buffer rewire to call `identifyPiece`, plus deletion of the legacy ngram/rerank/dtw surface) is a deferred slice tracked on #26, blocked on the #28 session-brain.ts rewrite.
