...
**Certified open-set gate WIRED INTO the SessionBrain DO (#26, closed):** The Phase-0-certified open-set piece-ID gate runs in Rust/WASM under `apps/api/src/wasm/piece-identify/` (`chroma.rs` for 12-bin chroma recall, `gate.rs` for the pitch-only chord-set Jaccard elastic-DTW margin gate @ threshold 0.0935, `identify.rs` for the chroma-recall→gate flow), exported as `identify_piece` and wrapped by `wasm-bridge.ts` as `identifyPiece`. The DO is now wired to it end-to-end: a cross-chunk `identificationNoteBuffer` (cap 1200 notes) accumulates perf notes from both the `eval_chunk` and `chunk_ready` paths; once the buffer reaches `MIN_NOTES_FOR_IDENTIFICATION`, `accumulateAndIdentify` runs `identifyPiece` (chroma recall → pitch-only Jaccard elastic-DTW margin gate @ 0.0935) over the accumulated buffer. On a confident margin it locks the piece (`pieceLocked` + `pieceIdentification{pieceId, confidence=margin, method:"identify_v2"}`); otherwise it stays unknown and the session degrades to Tier-3. The legacy three-stage ngram/rerank/dtw_confirm pipeline (Rust modules, the `wasm-bridge.ts` wrappers, and their TS types/interfaces) has been DELETED; the system now reads the v2 fingerprint artifact (`fingerprint/v2/piece_index.json`, generated by `just fingerprint`). A Rust↔Python port-fidelity harness (`parity_test.rs` vs committed golden `parity_fixtures.json`, 1e-4 tolerance, TA 14/16, OOD 0) guards the gate. **Production-deploy caveat:** the DO reads `fingerprint/v2/piece_index.json` from R2, so a production cutover is deploy-gated — prod R2 must receive the v2 artifact (local dev: `just fingerprint && just seed-fingerprint`). Until then production piece-ID degrades gracefully to unknown (R2 miss → null → Tier-3). Recall certification at scale (TA=0.875 is capped at the 16 amateur AMT recordings) remains a separate data-gathering follow-up; it fails gracefully (miss → unknown → Tier-3, never a wrong lock).
