...
| V6 harness loop | COMPLETE | `apps/api/src/harness/loop/` | Hook-driven two-phase compound execution loop. Phase 1 dispatches molecule atoms as Anthropic tools; Phase 2 forced-writes a Zod-validated SynthesisArtifact. V6 is the only synthesis path (the flag and the legacy path were deleted in #28). |
...
| V6 Integration (Plan 4) | COMPLETE | `apps/api/src/harness/` | ALL_ATOMS wired into OnSessionEnd, HookContext.digest populated with chunks/baselines/cohort_tables/session_history/past_diagnoses, diagnosis_artifacts table added, E2E test passes. V6 is the only synthesis path; the flag was deleted in #28. `buildGroundedDigest` (grounded-digest.ts) and `resolveMoleculeContext` (resolve-molecule-context.ts) added as deep modules: digest injected into synthesizeV6; past_diagnoses grounded server-side. Phase-1 and Phase-2 context overflow fixed via compact_signal_summary (no raw digest dump). (#99) |
...
| Transkun transcription | LOCAL ONLY | Transkun (MIT, ISMIR 2024) produces notes, offsets, and pedal CC64 events behind the frozen `/transcribe` contract. Prod `AMT_ENDPOINT=""` (unset) -> prod sessions degrade to Tier 3; local `.dev.vars` -> `localhost:8001`. Prod deploy deferred (pre-beta, local-first verification). |
