...
| Spec X: chat harness migration | COMPLETE | `apps/api/src/harness/`, `apps/api/src/routes/chat.ts` | chatV6 routes through runStreamingHook("OnChatMessage"); legacy chat() and buildChatBinding deleted. `HARNESS_V6_CHAT_ENABLED=true` in prod. |
