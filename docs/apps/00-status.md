...
| V6 molecules | COMPLETE | `apps/api/src/harness/skills/molecules/` | 8 ToolDefinition molecules (voicing-diagnosis, pedal-triage, rubato-coaching, phrasing-arc-analysis, tempo-stability-triage, dynamic-range-audit, articulation-clarity-check, cross-modal-contradiction-check) + `ALL_MOLECULES` barrel. exercise-proposal removed; exercise routing is now handled by the prescribe_exercise tool + pending_exercises table. Each molecule chains atoms deterministically and emits a Zod-validated DiagnosisArtifact. 25 tests (happy path + neutral path + insufficient-baseline exception paths). |
...
| AI Gateway | COMPLETE | `apps/api/src/services/llm.ts`, `apps/api/src/harness/loop/gateway-client.ts` | Unified authenticated CF AI Gateway (`crescendai`). Teacher defaults to Workers AI `@cf/qwen/qwen3-30b-a3b-fp8`; Anthropic selectable via `TEACHER_PROVIDER=anthropic`. Subagent unchanged (Workers AI). Dead vars `AI_GATEWAY_TEACHER`/`AI_GATEWAY_BACKGROUND`/`ANTHROPIC_API_KEY` removed; all gateway calls use `AI_GATEWAY_ENDPOINT` + `cf-aig-authorization`. (#61) |
...
| Two-stage LLM pipeline | Subagent (Workers AI/Gemma 4 26B) + Teacher (Anthropic/Sonnet 4.6) | Separates analysis (fast, cheap) from delivery (quality voice, ~1.5s). Different tasks need different models. |
