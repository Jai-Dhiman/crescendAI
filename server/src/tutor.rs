...
fn pick_weakest_dimensions(analysis: &AnalysisData, n: usize) -> Vec<(String, f32)> {
    let mut pairs = vec![
        ...
        (
            ...
            analysis.emotion_mood_optimistic_dark,
        ),
        (
            "emotion_mood_low_high_energy".to_string(),
            analysis.emotion_mood_low_high_energy,
        ),
        (
...
fn build_retrieval_query(analysis: &AnalysisData, user_ctx: &UserContext) -> String {
    let weakest = pick_weakest_dimensions(analysis, 4)
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let goals = if user_ctx.goals.is_empty() {
        "".to_string()
    } else {
        format!("; Goals: {}", user_ctx.goals.join(", "))
    };
    let constraints = if user_ctx.constraints.is_empty() {
        "".to_string()
    } else {
        format!("; Constraints: {}", user_ctx.constraints.join(", "))
    };
    let rep = if let Some(r) = &user_ctx.repertoire_info {
        if r.composer.is_empty() && r.piece.is_empty() {
            "".to_string()
        } else {
            format!("; Piece: {} {}", r.composer, r.piece)
        }
    } else {
        "".to_string()
    };
...
fn validate_and_normalize(mut feedback: TutorFeedback) -> TutorFeedback {
    ...
    let valid_ids: std::collections::HashSet<String> =
        feedback.citations.iter().map(|c| c.id.clone()).collect();
    for rec in feedback.recommendations.iter_mut() {
        ...
        if rec.estimated_time_minutes == 0 {
            rec.estimated_time_minutes = 10;
        }
        if rec.estimated_time_minutes > 120 {
            rec.estimated_time_minutes = 60;
        }
    }
    feedback
}
    ...
    if use_ace_pipeline {
        ...
        let config = AceConfig {
            ...
            curator_enabled: env
                ...
                .map(|v| v.to_string() == "true")
                .unwrap_or(true),
            max_playbook_size: env
                .var("ACE_MAX_PLAYBOOK_SIZE")
                .ok()
                .and_then(|v| v.to_string().parse().ok())
                .unwrap_or(500),
            confidence_threshold: env
                .var("ACE_CONFIDENCE_THRESHOLD")
                .ok()
                .and_then(|v| v.to_string().parse().ok())
                .unwrap_or(0.6),
            cache_ttl_hours: env
                .var("ACE_CACHE_TTL_HOURS")
                .ok()
                .and_then(|v| v.to_string().parse().ok())
                .unwrap_or(24),
        };
        ...
        let ace_output = ace_pipeline
            .generate_ace_feedback(env, analysis, user_ctx, Some(k), None)
            .await?;
            
        console_log!(
            "ACE pipeline completed in {}ms",
...
async fn generate_feedback_legacy(
    env: &Env,
    analysis: &AnalysisData,
    user_ctx: &UserContext,
    k: usize,
    ...
    if let Ok(kv) = env.kv("CRESCENDAI_METADATA") {
        if let Ok(json) = serde_json::to_string(&feedback) {
            let _ = kv
                .put(&format!("tutor:{}", cache_key), &json)?
...
pub async fn record_session_outcome(env: &Env, session_outcome: SessionOutcome) -> Result<()> {
    let config = AceConfig::default();
    let ace_pipeline = AcePipeline::with_config(config);

    ace_pipeline
        .record_session_outcome(env, session_outcome)
        .await
